...
def _pid_alive(pid: int) -> bool:
    ...
    **Zombies count as dead, and this is the whole point of the function.**
    ...
    Found 2026-09-24: ``@commitments-daily-0924-0645`` died with the
    session when the login expired, sat defunct for eight hours in status
    ``spawned:``, raised no ``!alert``, and Alex's morning brief simply
    never arrived. Two sibling agents died the same minute, were reaped
    normally, and alerted — the only difference was who held the pid entry.
    """
    try:
        os.kill(pid, 0)
